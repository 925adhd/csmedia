...
export default async function Home() {
  ...
  return (
    <>
      ...
      {/* Trust strip */}
      <section id="stats" className="scroll-mt-20 py-6 sm:py-8 bg-dark-800 border-y border-gold/15">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-3 text-center text-sm text-dark-100">
              <span className="inline-flex items-center gap-2">
                <span role="img" aria-label="Rated 5 out of 5 stars" className="inline-flex items-center gap-0.5">
                  {[...Array(5)].map((_, i) => (
                    <svg key={i} aria-hidden="true" className="w-3.5 h-3.5 text-gold" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                  ))}
                </span>
                <span>from Kentucky agents</span>
              </span>
              <span aria-hidden="true" className="hidden sm:inline text-gold/40">·</span>
